...
y=input2;
p=input3;
print("generator(alpha)","remainder(beta)","prime(p)");
print(g,y,p);
factors=factorint(p-1);
print("factors of p-1 are:");
print("%s" %  factors.keys())
ufactors=[];
index=0;

for i in factors.keys():
	ufactor=pow(i,factors[i]);
	ufactors.append(ufactor);	
	#print(ufactor);
	index=index+1;
print("unique factors are:%s" % ufactors);
print("subproblems are:");

with open('your_file.txt', 'w') as f:
	f.write(str(len(factors))+"\n");
	for i in ufactors:
		pi=i;
	#	print(pi);
		newg=pow(g,(p-1)/pi,p);
